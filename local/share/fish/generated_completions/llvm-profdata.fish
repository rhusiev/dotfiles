...
complete -c llvm-profdata -l convert-sample-profile-layout -d 'Convert the merged profile into a profile with a new layout'
...
complete -c llvm-profdata -l temporal-profile-trace-reservoir-size -d 'The maximum number of temporal profile traces to be stored in the output prof…'
complete -c llvm-profdata -l temporal-profile-max-trace-length -d 'The maximum number of functions in a single temporal profile trace'
