...
complete -c openvpn -l auth-nocache -d 'Don\'t cache --askpass or --auth-user-pass username/passwords in virtual memory'
...
complete -c openvpn -l compat-mode -d 'This option provides a convenient way to alter the defaults of OpenVPN to be …'
...
complete -c openvpn -l disable-dco -d 'Disable "data channel offload" (DCO)'
complete -c openvpn -l disable-occ -d 'DEPRECATED Disable "options consistency check" (OCC) in configurations that d…'
...
complete -c openvpn -l cipher -d 'This option should not be used any longer in TLS mode and still exists for tw…'
...
complete -c openvpn -l secret -d 'DEPRECATED Enable Static Key encryption mode (non-TLS)'
complete -c openvpn -l tran-window -d 'Transition window -- our old key can live this many seconds after a new a key…'
complete -c openvpn -l force-tls-key-material-export -d 'This option is only available in --mode server and forces to use Keying Mater…'
...
complete -c openvpn -l connect-retry -d 'Wait n seconds between connection attempts (default 1)'
...
complete -c openvpn -l dns -d 'Client DNS configuration to be used with the connection'
...
complete -c openvpn -l http-proxy-user-pass -d 'Overwrite the username/password information for --http-proxy'
...
complete -c openvpn -l connect-freq-initial -d '(UDP only) Allow a maximum of n initial connection packet responses per sec s…'
...
complete -c openvpn -l opt-verify -d 'DEPRECATED Clients that connect with options that are incompatible with those…'
...
complete -c openvpn -l stale-routes-check -d 'Remove routes which haven\'t had activity for n seconds (i. e'
...
complete -c openvpn -l rcvbuf -l session-timeout
...
complete -c openvpn -l tls-version-min -d 'Sets the minimum TLS version we will accept from the peer (default in 2. 6'
complete -c openvpn -l tls-version-max -d 'Set the maximum TLS version we will use (default is the highest version suppo…'
complete -c openvpn -l verify-hash -d 'DEPRECATED Specify SHA1 or SHA256 fingerprint for level-1 cert'
complete -c openvpn -l peer-fingerprint -d 'Specify a SHA256 fingerprint or list of SHA256 fingerprints to verify the pee…'
...
complete -c openvpn -l fragment -d 'Valid syntax: fragment max fragment max mtu Enable internal datagram fragment…'
complete -c openvpn -l keepalive -d 'A helper directive designed to simplify the expression of --ping and'
complete -c openvpn -l link-mtu -d 'DEPRECATED Sets an upper bound on the size of UDP packets which are sent betw…'
...
complete -c openvpn -l mssfix -d 'Valid syntax: mssfix max [mtu]'
complete -c openvpn -l max-packet-size -d 'work around cases where Path MTU discovery is broken on the network path betw…'
complete -c openvpn -l tun-mtu -d 'If the max-packet-size size option is used in the configuration it will also …'
...
complete -c openvpn -l dev-type
complete -c openvpn -l dev-node -d 'This is a highly system dependent option to influence tun/tap driver selection'
...
complete -c openvpn -l tun-max-mtu -d 'This configures the maximum MTU size that a server can push to maxmtu, by con…'
...
complete -c openvpn -l client-crresponse -d 'Execute in --mode server whenever a client sends a CR_RESPONSE message SCRIPT…'
...
complete -c openvpn -l tls-export-cert -d 'Adds an environment variable peer_cert when calling the'
...
complete -c openvpn -l management-client-pf -d 'Removed in OpenVPN 2. 6'
complete -c openvpn -l ncp-disable -d 'Removed in OpenVPN 2. 6'
...
complete -c openvpn -l prng -d 'Removed in OpenVPN 2. 6.   We now always use the PRNG of the SSL library'
